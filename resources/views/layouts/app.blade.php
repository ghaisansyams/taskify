...
<head>
    ...
    <title>@yield('title', 'Dashboard') — Taskify</title>
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 flex flex-col w-72 bg-white border-r border-slate-100 lg:translate-x-0">
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-5 space-y-0.5">
            ...
            <div class="pt-5 pb-1">
                ...
                <a href="{{ route('tasks.index', ['status' => 'todo']) }}" class="nav-status">
                    <span class="w-2 h-2 rounded-full bg-slate-400 shrink-0"></span>
                    To Do
                </a>
                <a href="{{ route('tasks.index', ['status' => 'in_progress']) }}" class="nav-status">
                    <span class="w-2 h-2 rounded-full bg-blue-500 shrink-0"></span>
                    In Progress
                </a>
                <a href="{{ route('tasks.index', ['status' => 'done']) }}" class="nav-status">
    ...
    <div class="flex-1 flex flex-col min-w-0 lg:ml-72">
        ...
        <header class="sticky top-0 z-20 h-16 bg-white border-b border-slate-100 flex items-center px-4 sm:px-6 gap-4">
            ...
            <div class="hidden sm:block">
                <h1 class="text-base font-bold text-slate-900">@yield('heading', 'Dashboard')</h1>
                <p class="text-xs text-slate-500 leading-none mt-0.5">@yield('subheading', 'Welcome back!')</p>
            </div>
            ...
            {{-- New Task CTA --}}
            <a href="{{ route('tasks.create') }}" class="btn-primary hidden sm:inline-flex text-sm py-2 px-4">
        ...
        <footer class="border-t border-slate-100 px-6 py-4">
            <p class="text-center text-xs text-slate-400">
                Taskify &copy; {{ date('Y') }} &mdash; Built with Laravel &amp; Blade
            </p>
        </footer>
    </div>
</div>
